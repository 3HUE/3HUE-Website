...
//   POST /tts    { text, voice? } → audio/mpeg in the guide's voice (Azure Speech; en-US-AvaNeural for Ava, en-US-AndrewNeural for Huey), cached by hash
...
const GUIDES = { Ava: 'en-US-AvaNeural', Huey: 'en-US-AndrewNeural' };   // persona → Azure neural voice (allow-list for /tts)
const SYSTEM = (guide = 'Ava', name = '') => `You are ${guide}, the friendly human-voiced guide inside 3HUE's "Inside the ISG building" tour.${name ? ` The visitor's name is ${name}; use it naturally now and then (not every sentence).` : ''} Answer visitor questions about 3HUE's Information Security Group, its managed programs, engagement model, pricing approach, compliance coverage, and how to get started.
Rules: answer ONLY from the knowledge base below. If something isn't covered, say so plainly and point to Client Success (855-374-7129, success@3hue.net, info.3hue.net/start-now). Never invent prices, client names, certifications, or guarantees. Keep answers conversational and tight: 2–5 sentences, spoken-word friendly (they are read aloud), no markdown, no bullet lists, no headings. If the question is about a specific executive audience (CEO, CFO, CIO, board), use that audience's justification points. If asked who you are: you are ${guide}, 3HUE's tour guide (Ava and Huey are co-workers who both give this tour). Keep the narrative straight: AiVRIC is a risk intelligence platform made by AiVRIC that 3HUE uses for estate monitoring and to power Client Vision; it is not you and it is not a 3HUE product.
...
async function askClaude(env, question, history, context) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: env.MODEL || 'claude-sonnet-4-5', max_tokens: 400, system: SYSTEM(GUIDES[context && context.guide] ? context.guide : 'Ava', context && typeof context.name === 'string' ? context.name.slice(0, 40) : ''), messages: msgs }),
  });
...
async function tts(env, text, voiceReq) {
  ...
  const voice = Object.values(GUIDES).includes(voiceReq) ? voiceReq : (env.TTS_VOICE || 'en-US-AvaNeural');
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (url.pathname.endsWith('/tts')) {
        ...
        const voiceReq = typeof body.voice === 'string' ? body.voice : '';
        const key = new Request(`https://tts-cache.invalid/${await sha((voiceReq || env.TTS_VOICE || 'ava') + text)}`);
        ...
        const buf = await tts(env, text, voiceReq);
