...
export function initHud(roomList, h) {
  ...
  if (strip) {
    ...
    for (const r of rooms) {
      const b = document.createElement('button');
      b.className = 'btn'; b.type = 'button'; b.textContent = r.short || r.name;
      b.style.setProperty('--dot', COLOR[r.streamColor] || 'var(--text-muted)');
      b.addEventListener('click', () => handlers.onRoom(r));
      strip.appendChild(b);
    }
...
export function updateHud({ view, room, station }) {
  ...
  if (!inRoom) { crumbsEl.innerHTML = ''; document.title = 'Inside 3HUE — Governance Acceleration & Enterprise Maturity'; return; }
  ...
  document.title = `${station ? station.name + ' · ' : ''}${room.name} — Inside 3HUE`;
}
