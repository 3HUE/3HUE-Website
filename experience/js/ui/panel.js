...
const STATUS_LABEL = { live: 'Live', beta: 'Beta', alpha: 'Alpha', roadmap: 'Roadmap', 'coming-soon': 'Coming soon', service: 'Managed program', platform: 'Powered by AiVRIC' };
let onSelectStation = () => {};
...
function renderStation(room, s) {
  ...
  const ctas = (s.links || []).map((l) =>
    `<a class="btn ${l.primary ? 'primary' : 'outline'}" href="${esc(l.href)}"${l.href.startsWith('#') ? '' : ' target="_blank" rel="noopener"'}>${esc(l.label)}
      ${l.primary || l.href.startsWith('#') ? '' : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 4h6v6M20 4l-9 9M19 14v5a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h5"/></svg>'}
    </a>`).join('');
